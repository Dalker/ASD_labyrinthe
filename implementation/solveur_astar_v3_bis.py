...
Cette version "bis" est une version "pure" ne pouvant pas être
...
def astar(grid, distance=manhattan_distance):
    """
...
def profiling():
    """Effectuer test avec profiling."""
    import cProfile
    from pstats import SortKey
    print("- profiling with null distance -")
    cProfile.run("astar(maze, distance=null_distance)", sort=SortKey.TIME)
    print("- profiling with Manhattan distance -")
    cProfile.run("astar(maze, distance=manhattan_distance)", sort=SortKey.TIME)
