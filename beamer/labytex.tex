...
\begin{frame}
  ...
  \begin{block}{pseudo-code}
    ...
    \item<4-> si $C$ est la cellule d'arrivée $A$, retourner le chemin qui y amène (via backtracking sur les prédecesseurs)
    \item<5-> sinon, pour chaque voisin $V$ de $C$ qui n'est pas déjà accessible à moindre coût
      \begin{itemize}
      \item<6-> mémoriser le prédecesseur de $V$ (donc $C$) et le coût d'accès à $V$
      \item<7-> insérer (push) $V$ dans la file d'attente
      \end{itemize}
    \end{itemize}    
  \end{block}
\end{frame}
...
% Une image serait pratique pour la tringulation etc.
    \par\smallskip
...
\begin{frame}
  \frametitle{Complexité de l'algorithme - 1}
...
\begin{frame}
  ...
  \begin{minipage}{.35\textwidth}    
   Par Johann Dréo (User:Nojhan) — Travail personnel, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=815564
  \end{minipage}
\end{frame}
...
\begin{frame}
  ...
  Robots aspirateurs
  \onslide<2->\begin{itemize} \item But un peu différent car ils doivent passer partout 1 seule fois
  \onslide<3->\item Mais ils doivent aussi trouver leur chemin d'un point A vers un point B
  \onslide<4->\item Ils utilisent souvent des algorithmes évolutionnistes
  \onslide<5->\item Gènes = paramètres à optimiser (chemins) 
  \onslide<6->\item Transmission de gènes, via reproduction
  \onslide<7->\item Quelques mutations puis verdict du calcul d'adaptabilité
  \onslide<8->\item Les "individus" moins performants disparaissent
  \onslide<9->\item Avantage: Adaptable si le labyrinthe se modifie
  \onslide<10->\item Désavantage: Beaucoup d'individus et de générations pour parvenir à un bon résultat  
  \end{itemize}
\end{frame}
