...
% "admissibilité" ??? Plutôt "sous-estimation", ou qqch dans ce genre?
    ...
    \onslide<5->{Par contradiction:}\onslide<6->{s'il y en avait un, il aurait
      ...
      a une priorité calculée en coût réel. (coût heuristique de cellule A $ \le $ 0 !)}}
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{minipage}{.55\textwidth}
    ...
    \only<3>{Estimation de la distance réelle de l'arrivée pour chaque cellule (Heuristique "Manhattan").}
...
\subsection{Complexité - algorithme abstrait}
\begin{frame}
