...
\begin{frame}
  ...
  L'heuristique $h(C,S)$ qui estime le chemin restant doit satisfaire deux conditions.
  \begin{enumerate}
  \item<2-> \textbf{l'inégalité trianguaire}:
    $h(C_1,S) \leq r(C_1,C_2) + h(C_2, S)$ où $C_1, C_2$ sont deux cellules,
    $r(C_1, C_2)$ et la distance réelle entre elles.
    \par\smallskip
    \onslide<2->{Elle garantit de trouver la sortie, sans se ``perdre'' dans des
      boucles éventuelles.}
  \item<3-> \textbf{l'heuristique ne sur-estime jamais une distance}
    \par\smallskip
